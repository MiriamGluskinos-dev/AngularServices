...
@Component({
  selector: 'app-search-table-final',
  templateUrl: './search-table-final.component.html',
  styleUrls: ['./search-table-final.component.css']
})
export class SearchTableFinalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // //פניה לשירות לשליפת הנתונים
    // this.httpService.get<ConcentratedServiceData>("datastore_search",
    //   "https://data.gov.il/api/action/",
    //   new HttpParams().set('resource_id', '9f5cacc9-a2cc-484f-aa7f-ad55160a1d39').set('limit', '1000'))
    //   .subscribe((data: ConcentratedServiceData) => {

    //     //איתחול הנתונים בתוך מערך
    //     this.gridData = data.result.records.map(item =>
    //       new ConcentratedData(item.ID, item.Category1, item.Category2, item.product, item.UpTo75_, item.From75_To500_, item.From500_To1000_, item.Over1000_));
    ...
    // this.cols = [
    //   { field: 'Category1', header: 'קטגוריה ראשית' },
    //   { field: 'Category2', header: 'קטגוריה משנית' },
    //   { field: 'product', header: 'מוצר' },
    //   { field: 'UpTo75_', header: 'עד 75$' },
    //   { field: 'From500_To1000_', header: 'מ76$ עד 500$' },
    //   { field: 'From500_To1000_', header: 'מ501$ עד 1000$' },
    //   { field: 'Over1000_', header: 'מעל 1000$' },
    ...
    //   new ConcentratedData("1", "חשמל ואלקטרוניקה", "מערכות צפיה ושמע", "טלויזיה עד 50 אינטש", "פטור", "17.00%", "שיעור 34.55%", "17.00%"),
    //   new ConcentratedData("2", "חשמל ואלקטרוניקה", "מערכות צפיה ושמע", "טלויזיה מעל 50 אינטש", "פטור", "17.00%", "שיעור 34.55%", "17.00%"),
    //   new ConcentratedData("3", "חשמל ואלקטרוניקה", "מערכות צפיה ושמע", "אוזניות", "פטור", "17.00%", "שיעור 34.55%", "17.00%")
    // ];
    //עד כאן היה בהערה
  }
  ...
  private onSuccess(data: any) {
    ...
    this.gridData = data['Content'].map(item =>
